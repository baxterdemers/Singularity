...
const Agents = (props) => {
  ...
  const getMaybeReactivateButton = (agent) => (
    Utils.isIn(agent.currentState.state, ['DECOMMISSIONING', 'DECOMMISSIONED', 'STARTING_DECOMMISSION', 'FROZEN']) && (
      <FormModalButton
        ...
        <pre>{agent.id}</pre>
        <p>Reactivating a agent will cancel the decommission without erasing the agent's history and move it back to the active state.</p>
      </FormModalButton>
  ));
  ...
  const getMaybeFreezeButton = (agent) => (agent.currentState.state === 'ACTIVE' &&
    <FormModalButton
      ...
      <pre>{agent.id}</pre>
      <p>Freezing a agent will prevent new tasks from being launched. Previously running tasks will be unaffected.</p>
    </FormModalButton>
  );
  ...
  const getMaybeDecommissionButton = (agent) => (Utils.isIn(agent.currentState.state, ['ACTIVE', 'FROZEN']) && (
    <FormModalButton
      ...
      <pre>{agent.id}</pre>
      <p>Decommissioning a agent causes all tasks currently running on it to be rescheduled and executed elsewhere,
